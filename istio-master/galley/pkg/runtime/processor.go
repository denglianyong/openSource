...
// 从提供者监听到来的事件，通过Distributor 发布组件配置
func (p *Processor) Start() error {
	...
	p.distribute = false
//将item加入 channel    F:\go-source\istio-master\galley\pkg\kube\source\source.go
	...
	p.events = events
	// F:\go-source\istio-master\galley\pkg\runtime\state.go  
...
loop:
	for {
		...
		//F:\go-source\istio-master\galley\pkg\kube\source\source.go  从k8s上获取事件
		case e := <-p.events:
...
//将事件存入内存而已 F:\go-source\istio-master\galley\pkg\runtime\state.go
