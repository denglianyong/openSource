...
func NewServer(args PilotArgs) (*Server, error) {
	...
	if args.CtrlZOptions != nil {
		//F:\go-source\istio-master\pkg\ctrlz\ctrlz.go
...
func (s *Server) initConfigController(args *PilotArgs) error {
	if args.Config.Controller != nil {
		...
		//F:\go-source\istio-master\pilot\pkg\config\memory\controller.go
		...
		if args.Config.CFConfig != "" {
			...
			confController, err := configaggregate.MakeCache([]model.ConfigStoreCache{
				s.configController,
				// F:\go-source\istio-master\pilot\pkg\config\cloudfoundry\controller.go
				// 获取路由规则
		...
		// 自定义资源管理器，并监视 F:\go-source\istio-master\pilot\pkg\config\kube\crd\controller.go
	...
	if hasKubeRegistry(args) && s.mesh.IngressControllerMode != meshconfig.MeshConfig_OFF {
		...
		configController, err := configaggregate.MakeCache([]model.ConfigStoreCache{
			s.configController,
			//F:\go-source\istio-master\pilot\pkg\config\kube\ingress\controller.go
			// 监听所有ingress
...
func (s *Server) makeKubeConfigController(args *PilotArgs) (model.ConfigStoreCache, error) {
	kubeCfgFile := s.getKubeCfgFile(args)
	//F:\go-source\istio-master\pilot\pkg\config\kube\crd\client.go
	configClient, err := crd.NewClient(kubeCfgFile, "", model.IstioConfigTypes, args.Config.ControllerOptions.DomainSuffix)
	if err != nil {
		return nil, multierror.Prefix(err, "failed to open a config client.")
	}
// 向k8s注册自定义资源  F:\go-source\istio-master\pilot\pkg\config\kube\crd\client.go
	if err = configClient.RegisterResources(); err != nil {
		return nil, multierror.Prefix(err, "failed to register custom resources.")
	}
//F:\go-source\istio-master\pilot\pkg\config\kube\crd\controller.go   返回一个监听器
...
func (s *Server) initMultiClusterController(args *PilotArgs) (err error) {
	if checkForKubernetes(args.Service.Registries) {
		...
		// 启动controller 监视运行时 secret，动态的增加 集群
...
func (s *Server) initServiceControllers(args *PilotArgs) error {
	// F:\go-source\istio-master\pilot\pkg\serviceregistry\aggregate\controller.go 
	// 服务注册中心控制器
	serviceControllers := aggregate.NewController()
	registered := make(map[serviceregistry.ServiceRegistry]bool)
	for _, r := range args.Service.Registries {
		// F:\go-source\istio-master\pilot\pkg\serviceregistry\platform.go 就是一个字符串
	...
	//外部服务发现  F:\go-source\istio-master\pilot\pkg\serviceregistry\external\servicediscovery.go
...
func (s *Server) initDiscoveryService(args *PilotArgs) error {
	...
	// F:\go-source\istio-master\pilot\pkg\proxy\envoy\discovery.go
	// 启动注册服务的HTTP server， 注册一堆事件处理器 instance  service  handler
	// 将service发送给
	...
	s.initGrpcServer()
//F:\go-source\istio-master\pilot\pkg\proxy\envoy\v2\discovery.go 
//  xds都实现了 GRPC接口，提供 discovery服务
	...
	// ads  apis 通过grpc 暴露出去  //注册 ADS EDS 处理  envoy发来的请求
