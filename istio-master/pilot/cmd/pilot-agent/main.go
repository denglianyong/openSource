...
var (
	...
	proxyCmd = &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			//F:\go-source\istio-master\pilot\pkg\proxy\envoy\watcher.go
			envoyProxy := envoy.NewProxy(proxyConfig, role.ServiceNode(), proxyLogLevel, pilotSAN)
			//  envoy上的代理，负责重启  F:\go-source\istio-master\pilot\pkg\proxy\agent.go
			agent := proxy.NewAgent(envoyProxy, proxy.DefaultRetry)
			//F:\go-source\istio-master\pilot\pkg\proxy\envoy\watcher.go 用agent作为watcher监视envoy，重新加载配置
			// watcher是在agent上再包装一层 
