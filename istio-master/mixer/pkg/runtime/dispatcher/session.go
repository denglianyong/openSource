...
func (s *session) dispatch() error {
	...
	//查找对应的适配器 Routes =table.go  F:\go-source\istio-master\mixer\pkg\runtime\routing\table.go
	...
	for _, destination := range destinations.Entries() {
		...
		if s.variety == tpb.TEMPLATE_VARIETY_REPORT {
			...
			if state == nil {
				//impl===dispatcher  F:\go-source\istio-master\mixer\pkg\runtime\dispatcher\dispatcher.go
...
func (s *session) dispatchToHandler(ds *dispatchState) {
	s.activeDispatches++
	ds.session = s
	//F:\go-source\istio-master\mixer\pkg\pool\goroutine.go
	// 通过协程池进行调度
