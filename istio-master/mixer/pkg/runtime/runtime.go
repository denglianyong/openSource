...
func New(
	...
	//F:\go-source\istio-master\mixer\pkg\runtime\config\ephemeral.go
	//   编译器，builder  
	e := config.NewEphemeral(templates, adapters)
	rt := &Runtime{
		defaultConfigNamespace: defaultConfigNamespace,
		ephemeral:              e,
		snapshot:               config.Empty(),//F:\go-source\istio-master\mixer\pkg\runtime\config\snapshot.go
		handlers:               handler.Empty(),//F:\go-source\istio-master\mixer\pkg\runtime\config\handler.go
...
// 检查配置是否改变
//F:\go-source\istio-master\mixer\pkg\runtime\config\kinds.go
	kinds := config.KindMap(c.snapshot.Adapters, c.snapshot.Templates)
//F:\go-source\istio-master\mixer\pkg\config\store\listener.go
//从 F:\go-source\istio-master\mixer\pkg\config\mcp\backend.go  
// 从F:\go-source\istio-master\mixer\pkg\config\crd\store.go watch  galley的配置
	...
	go func() {
		//watchFlushDuration  调用 onConfigChange
		//F:\go-source\istio-master\mixer\pkg\config\store\listener.go
...
//F:\go-source\istio-master\mixer\pkg\runtime\handler\table.go
	newHandlers := handler.NewTable(oldHandlers, newSnapshot, c.handlerPool)
//F:\go-source\istio-master\mixer\pkg\lang\compiled\expressionbuilder.go
	builder := compiled.NewBuilder(newSnapshot.Attributes)
	//路由表  F:\go-source\istio-master\mixer\pkg\runtime\routing\builder.go
	// 查找对应适配器 
	newRoutes := routing.BuildTable(
		newHandlers, newSnapshot, builder, c.defaultConfigNamespace, log.DebugEnabled())
	//切换新的路由表  F:\go-source\istio-master\mixer\pkg\runtime\dispatcher\dispatcher.go
