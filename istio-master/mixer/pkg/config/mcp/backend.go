...
//F:\go-source\istio-master\mixer\pkg\config\inventory.go 调用
...
//F:\go-source\istio-master\mixer\pkg\config\store\listener.go  回调
...
// 连接MCP server  F:\go-source\istio-master\pkg\mcp\server\server.go
...
// 连接MCP
	...
	//F:\go-source\istio-master\pkg\mcp\client\client.go
	c := client.New(cl, messageNames, b, mixerNodeID, map[string]string{})
	//F:\go-source\istio-master\pkg\mcp\configz\configz.go
	// 注册topic, 生车MCP client状态信息供查看 
	configz.Register(c)
...
//F:\go-source\istio-master\mixer\pkg\config\store\listener.go 通过该函数取走配置
