...
InstanceImpl::InstanceImpl(Options& options, Event::TimeSystem& time_system,
    ...
    : options_(options), time_system_(time_system), restarter_(restarter),
      ...
      //  主角 分发器 libevent E:\envoy-master\source\common\api\api_impl.cc
      //E:\envoy-master\source\common\event\dispatcher_impl.cc 相当于reactor
      dispatcher_(api_->allocateDispatcher(time_system)),


      singleton_manager_(new Singleton::ManagerImpl()),
      //  主角--连接管理器   E:\envoy-master\source\server\connection_handler_impl.cc
      handler_(new ConnectionHandlerImpl(ENVOY_LOGGER(), *dispatcher_)),
      random_generator_(std::move(random_generator)),
      secret_manager_(std::make_unique<Secret::SecretManagerImpl>()),
      // 监听工作线程  可能在  E:\envoy-master\source\server\listener_manager_impl.cc
...
void InstanceImpl::initialize(Options& options,
  ...
  // E:\envoy-master\source\server\http\admin.cc  暴露统计数据
  ...
  if (initial_config.admin().address()) {
    //E:\envoy-master\source\server\connection_handler_impl.cc
  ...
  // E:\envoy-master\source\server\listener_manager_impl.cc
  // 管理各种监听器,管理端口，代理应用程序端口，LDS端口
  // 调用factory方法，创建一大堆 filter
  listener_manager_.reset(
      new ListenerManagerImpl(*this, listener_component_factory_, worker_factory_, time_system_));

  // The main thread is also registered for thread local updates so that code that does not care
  // whether it runs on the main thread or on workers can still use TLS.
  // E:\envoy-master\source\common\event\dispatcher_impl.cc  事件调度器
  thread_local_.registerThread(*dispatcher_, true);

  // Initialize the overload manager early so other modules can register for actions.
  // E:\envoy-master\source\server\overload_manager_impl.cc
  //  重载管理器  方便注册，watchDog
...
//E:\envoy-master\source\common\upstream\cluster_manager_impl.cc 集群管理
// 上游service的集群，有个疑问，所有集群都同步过来?? 还是按需同步
  ...
  // 解析主配置文件  E:\envoy-master\source\server\configuration_impl.cc
  //这里似乎有连接mixer  metric
  Configuration::MainImpl* main_config = new Configuration::MainImpl();
  config_.reset(main_config);
  // 启动监听器，处理 业务程序发送过来的请求,创建 upstream的 cluster
  main_config->initialize(bootstrap_, *this, *cluster_manager_factory_);

  // Instruct the listener manager to create the LDS provider if needed. This must be done later
  // because various items do not yet exist when the listener manager is created.
  if (bootstrap_.dynamic_resources().has_lds_config()) {
    //E:\envoy-master\source\server\listener_manager_impl.h  写在头文件里  通过http请求获取LDS
  ...
  if (bootstrap_.has_hds_config()) {
    ...
    async_client_manager_ = std::make_unique<Grpc::AsyncClientManagerImpl>(
     ...
     // E:\envoy-master\source\common\upstream\health_discovery_service.cc upstream  健康检查 服务发现   
...
void InstanceImpl::run() {
  ...
  run_helper_ = std::make_unique<RunHelper>(*dispatcher_, clusterManager(), restarter_,
                                            ...
                                            //  启动工作线程,开始监听--重点关注 
//每个work绑定一个单独的ConnectionHandler，handler将完成listener绑定的任务。
//worker即代表了envoy服务的并发度。主体逻辑都将在worker及绑定的对应的libevent库中执行。   
//有init_manager_  来回调   ==> startWorkers                               
