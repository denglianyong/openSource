...
ConnectionHandlerImpl::ActiveListener::ActiveListener(ConnectionHandlerImpl& parent,
    ...
    : ActiveListener(
          parent,
          //   E:\envoy-master\source\common\event\dispatcher_impl.cc
          // 创建监听器的实现  ListenerImpl 在E:\envoy-master\source\common\event\dispatcher_impl.cc
          // ConnectionHandlerImpl::ActiveListener 作为cb 等待这个fd有数据
...
void ConnectionHandlerImpl::ActiveSocket::continueFilterChain(bool success) {
  if (success) {
    ...
    if (new_listener != nullptr) {
      ...
      //listener->activeListener ->  connection 
...
// network的 listener_impl 的callback  E:\envoy-master\source\common\network\listener_impl.cc
void ConnectionHandlerImpl::ActiveListener::onAccept(
  ...
  // Create and run the filters  创建filter
...
void ConnectionHandlerImpl::ActiveListener::newConnection(Network::ConnectionSocketPtr&& socket) {
  ...
  //E:\envoy-master\source\server\listener_manager_impl.cc  似乎与路由相关的设置
...
// 创建ssl的context， 也没干别的
  ...
  // 把2个socket整合在一起,新建连接
  //E:\envoy-master\source\common\event\dispatcher_impl.cc 
  // 创建新connection向libevent注册  回调函数-- 构造函数中顺便完成注册;关节函数
  // 最终在E:\envoy-master\source\common\network\connection_impl.cc 创建连接
...
ConnectionHandlerImpl::ActiveConnection::ActiveConnection(ActiveListener& listener,
  ...
  // 关节点， handler作为connection的eventListner
