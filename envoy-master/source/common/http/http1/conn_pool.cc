...
void ConnPoolImpl::attachRequestToClient(ActiveClient& client, StreamDecoder& response_decoder,
  ...
  ASSERT(!client.stream_wrapper_);
      //  response_decoder ,callbacks  都==Filter::UpstreamRequest  
  client.stream_wrapper_.reset(new StreamWrapper(response_decoder, client));
    //  response_decoder ,callbacks  都==Filter::UpstreamRequest  
    // 所以回调  UpstreamRequest.onPoolReady  E:\envoy-master\source\common\router\router.cc
...
//E:\envoy-master\source\common\router\router.cc  ==Filter::UpstreamRequest 
//void Filter::UpstreamRequest::encodeHeaders 在解析downstream的 header时调用
ConnectionPool::Cancellable* ConnPoolImpl::newStream(StreamDecoder& response_decoder,
  ...
  if (!ready_clients_.empty()) {
    ...
    // callbacks == 见E:\envoy-master\source\common\router\router.cc  ==Filter::UpstreamRequest
    //  response_decoder ,callbacks  都==Filter::UpstreamRequest  
...
//  response_decoder ==Filter::UpstreamRequest  
ConnPoolImpl::StreamWrapper::StreamWrapper(StreamDecoder& response_decoder, ActiveClient& parent)
// 建立upstream的 stream        codec_client_->newStream 在 E:\envoy-master\source\common\http\http1\codec_impl.cc
// 上游的编码器
    : StreamEncoderWrapper(parent.codec_client_->newStream(*this)),
    // response_decoder=== E:\envoy-master\source\common\router\router.cc  Filter::UpstreamRequest 
    //  将上游的响应解码器  发给downstream 
      StreamDecoderWrapper(response_decoder), parent_(parent) {
// inner   == RequestStreamEncoderImpl是一个 ServerConnectionImpl; getStream()===StreamEncoderImpl 
//  这个回调函数 似乎也无关紧要
...
// E:\envoy-master\source\common\http\codec_wrappers.h 回调 Filter::UpstreamRequest 
