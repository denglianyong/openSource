...
//  山重水复疑无路 ；柳暗花明=== 本文件 http_parser_settings ConnectionImpl::settings_
...
//http_parser_settings ConnectionImpl::settings_{ 中在解析http请求时 回调。流程又串起来了
...
//callbacks===E:\envoy-master\source\common\http\conn_manager_impl.cc 
...
void ServerConnectionImpl::onMessageBegin() {
  if (!resetStreamCalled()) {
    ...
    //callbacks===E:\envoy-master\source\common\http\conn_manager_impl.cc 
    //  active_request_->response_encoder_=== ServerConnectionImpl
    active_request_->request_decoder_ = &callbacks_.newStream(active_request_->response_encoder_);
    //active_request_->request_decoder_   ==ActiveStream  E:\envoy-master\source\common\http\conn_manager_impl.cc
  }
}
...
void ServerConnectionImpl::onMessageComplete() {
  if (active_request_) {
    ...
    if (deferred_end_stream_headers_) {
      ...
      //E:\envoy-master\source\common\http\conn_manager_impl.cc
...
//  解码 upstream的http 响应;在哪里调用？
// ans   ConnPoolImpl::StreamWrapper::StreamWrapper(StreamDecoder& response_decoder, ActiveClient& parent)
//代码在 E:\envoy-master\source\common\http\http1\conn_pool.cc 
StreamEncoder& ClientConnectionImpl::newStream(StreamDecoder& response_decoder) {
  ...
  // response_decoder=== ConnPoolImpl::StreamWrapper  做为上游的解码器
...
int ClientConnectionImpl::onHeadersComplete(HeaderMapImplPtr&& headers) {
  ...
  if (pending_responses_.empty() && !resetStreamCalled()) {
    ...
    if (parser_.status_code == 100) {
      ...
      //  调用  StreamDecoderWrapper   将响应发送给downstream
      //E:\envoy-master\source\common\http\codec_wrappers.h
...
void ClientConnectionImpl::onMessageComplete() {
  ...
  if (!pending_responses_.empty()) {
    ...
    if (deferred_end_stream_headers_) {
      //      //E:\envoy-master\source\common\http\codec_wrappers.h  StreamDecoderWrapper
