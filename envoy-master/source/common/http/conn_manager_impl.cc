...
// E:\envoy-master\source\common\network\filter_manager_impl.cc 中调用 
// callbacks==
void ConnectionManagerImpl::initializeReadFilterCallbacks(Network::ReadFilterCallbacks& callbacks) {
 
 
  //E:\envoy-master\source\common\network\filter_manager_impl.cc callbacks==  ActiveReadFilter
...
// 设置connection的eventListener 回调函数   似乎是支流
...
//E:\envoy-master\source\common\http\http1\codec_impl.cc  在解析downstream的开始时调用 创建stream
...
StreamDecoder& ConnectionManagerImpl::newStream(StreamEncoder& response_encoder) {
  ...
  //    response_encoder  === ServerConnectionImpl   在 E:\envoy-master\source\common\http\http1\codec_impl.cc
  // 将upstream的数据 发给 downstream
...
//  E:\envoy-master\source\common\network\connection_impl.cc  ===回调 这个函数???
Network::FilterStatus ConnectionManagerImpl::onData(Buffer::Instance& data, bool end_stream) {
  ...
  if (!codec_) {
    ...
    // ServerConnectionImpl  E:\envoy-master\source\extensions\filters\network\http_connection_manager\config.cc
    //  read_callbacks_->connection() == E:\envoy-master\source\common\network\connection_impl.cc
    // codec_ =ServerConnectionImpl E:\envoy-master\source\common\network\connection_impl.cc
  ...
  do {
    ...
    try {
      //E:\envoy-master\source\common\http\http2\codec_impl.cc
...
//  增加stream的 filter  E:\envoy-master\source\extensions\filters\http\router\config.cc 调用这个函数，增加filter
// 同时看  .h文件
void ConnectionManagerImpl::ActiveStream::addStreamDecoderFilterWorker(
  ...
  // filter  实际上 == E:\envoy-master\source\common\router\router.cc
...
void ConnectionManagerImpl::ActiveStream::decodeHeaders(ActiveStreamDecoderFilter* filter,
  ...
  for (; entry != decoder_filters_.end(); entry++) {
    ...
    //  查找集群，路由  等 E:\envoy-master\source\common\router\router.cc
...
void ConnectionManagerImpl::ActiveStream::decodeData(ActiveStreamDecoderFilter* filter,
  ...
  for (; entry != decoder_filters_.end(); entry++) {
    ...
    //E:\envoy-master\source\extensions\filters\http\router\config.cc 工厂方法在这里
...
void ConnectionManagerImpl::ActiveStream::decodeTrailers(ActiveStreamDecoderFilter* filter,
  ...
  std::list<ActiveStreamDecoderFilterPtr>::iterator entry;
  if (!filter) {
    // wrapper  ActiveStreamDecoderFilter
    entry = decoder_filters_.begin();
  } else {
    entry = std::next(filter->entry());
  }
...
// 将upstream返回的数据发给downstream 
//E:\envoy-master\source\common\router\router.cc 中  void Filter::onUpstreamData 回调   callbacks_->encodeData(data, end_stream);
void ConnectionManagerImpl::ActiveStream::encodeData(ActiveStreamEncoderFilter* filter,
  ...
  if (trailers_added_entry != encoder_filters_.end()) {
    // 返回给downstream  // active_request_->response_encoder_=== ServerConnectionImpl
    // E:\envoy-master\source\common\http\http1\codec_impl.cc
...
// active_request_->response_encoder_=== ServerConnectionImpl  E:\envoy-master\source\common\http\http1\codec_impl.cc
