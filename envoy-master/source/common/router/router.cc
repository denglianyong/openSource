...
Http::FilterHeadersStatus Filter::decodeHeaders(Http::HeaderMap& headers, bool end_stream) {
  ...
  // Fetch a connection pool for the upstream cluster. 查找集群的连接池---重点关注
...
// 建立 upstream的请求，finilize时，发送tracer给mixer
...
//E:\envoy-master\source\common\http\conn_manager_impl.cc   === new ActiveStreamDecoderFilter
void Filter::setDecoderFilterCallbacks(Http::StreamDecoderFilterCallbacks& callbacks) {

  callbacks_ = &callbacks;  // ActiveStreamDecoderFilter==ConnectionManagerImpl::ActiveStream 奇怪的语法
...
void Filter::maybeDoShadowing() {
  ...
  Http::MessagePtr request(new Http::RequestMessageImpl(
      ...
      //ConnectionManagerImpl::ActiveStream   E:\envoy-master\source\common\http\conn_manager_impl.cc
...
// ConnectionManagerImpl::ActiveStream::encode100ContinueHeaders
//  //ConnectionManagerImpl::ActiveStream   E:\envoy-master\source\common\http\conn_manager_impl.cc
...
void Filter::onUpstreamHeaders(const uint64_t response_code, Http::HeaderMapPtr&& headers,
  ...
  //E:\envoy-master\source\common\http\conn_manager_impl.cc ConnectionManagerImpl::ActiveStream:
      callbacks_->requestInfo().setResponseFlag(RequestInfo::ResponseFlag::UpstreamOverflow);
    }
...
//E:\envoy-master\source\common\http\conn_manager_impl.cc ConnectionManagerImpl::ActiveStream::encodeHeaders
  callbacks_->encodeHeaders(std::move(headers), end_stream);
}

//  这里应该是讲upstream的数据  返回downstream
...
Filter::UpstreamRequest::~UpstreamRequest() {
  if (span_ != nullptr) {
    ...
    // 发送 给mixer  E:\envoy-master\source\extensions\tracers\zipkin\zipkin_tracer_impl.cc
    span_->finishSpan();
  }
...
// 还有一个问题， UpstreamRequest在哪里将自己做的callback????
//  E:\envoy-master\source\common\http\http1\conn_pool.cc 中 ConnectionPool::Cancellable* ConnPoolImpl::newStream  
//这里是连接池的监听器，不是数据回调触发点：  
// 将upstreamRequest 作为回调函数 存在upstream 的client中;;; 
//功能： 解码upstream返回的数据
void Filter::UpstreamRequest::decodeData(Buffer::Instance& data, bool end_stream) {
  maybeEndDecode(end_stream);
  request_info_.addBytesReceived(data.length());
  //637 行将数据发回给 downstream
...
void Filter::UpstreamRequest::encodeHeaders(bool end_stream) {
  ...
  // with this case.
  // 这里终于见到 stream上的stream了  E:\envoy-master\source\common\http\http1\conn_pool.cc
  // 第二个参数  void Filter::UpstreamRequest 实现 ConnectionPool::Callbacks& callbacks
...
void Filter::UpstreamRequest::encodeData(Buffer::Instance& data, bool end_stream) {
  ...
  if (!request_encoder_) {
    ...
    //request_encoder  == new StreamWrapper(response_decoder, client)  E:\envoy-master\source\common\http\http1\conn_pool.cc
    //  父类StreamEncoderWrapper 实现  encodeData  E:\envoy-master\source\common\http\codec_wrappers.h
    // 最终跳转到E:\envoy-master\source\common\http\http1\codec_impl.cc  
    // request_encoder_==  StreamEncoderImpl  RequestStreamEncoderImpl
    request_encoder_->encodeData(data, end_stream);
    if (end_stream) {
      request_info_.onLastUpstreamTxByteSent();
      // callbacks_=ConnectionManagerImpl::ActiveStreamDecoderFilter:  E:\envoy-master\source\common\http\conn_manager_impl.cc
...
void Filter::UpstreamRequest::encodeTrailers(const Http::HeaderMap& trailers) {
  ...
  if (!request_encoder_) {
    ...
    ENVOY_STREAM_LOG(trace, "proxying trailers", *parent_.callbacks_);
       ...
       //E:\envoy-master\source\common\http\http1\conn_pool.cc  上游编码器
       //最终E:\envoy-master\source\common\http\http1\codec_impl.cc
...
// Filter::UpstreamRequest 实现 ConnectionPool::Callbacks& callbacks 
//E:\envoy-master\source\common\http\http1\conn_pool.cc 中回调
void Filter::UpstreamRequest::onPoolReady(Http::StreamEncoder& request_encoder,
  ...
  onUpstreamHostSelected(host);
  // request_encoder  == new StreamWrapper(response_decoder, client)  E:\envoy-master\source\common\http\http1\conn_pool.cc
  // getStream== ServerConnectionImpl
  ...
  parent_.callbacks_->requestInfo().onFirstUpstreamTxByteSent();
        ...
        //E:\envoy-master\source\common\http\http1\conn_pool.cc  上游编码器
       //最终E:\envoy-master\source\common\http\http1\codec_impl.cc  RequestStreamEncoderImpl
...
void Filter::UpstreamRequest::setRequestEncoder(Http::StreamEncoder& request_encoder) {
  // request_encoder  == new StreamWrapper(response_decoder, client)  E:\envoy-master\source\common\http\http1\conn_pool.cc
  //上游编码器 RequestStreamEncoderImpl 
  ...
  // addDownstreamWatermarkCallbacks 在E:\envoy-master\source\common\http\conn_manager_impl.cc实现
  // callbacks_=ConnectionManagerImpl::ActiveStreamDecoderFilter:  E:\envoy-master\source\common\http\conn_manager_impl.cc
