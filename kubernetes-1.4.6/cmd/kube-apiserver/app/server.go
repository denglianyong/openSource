...
func Run(s *options.APIServer) error {
	...
	if len(s.SSHUser) > 0 {
		...
		//	kubernetes 提供对aws、azure、gce、cloudstack、mesos等云供应商提供了接口支持，目前包括负载均衡、实例、zone信息、路由信息等
	...
	// 连接到 kubelet   master 和 node 是平等的   F:\go-source\kubernetes-1.4.6\pkg\kubelet\client\kubelet_client.go 
	//kubelet 的健康检查
	...
	//存储  F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\default_storage_factory_builder.go
	...
	// F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\storage_factory.go
	...
	if modeEnabled(genericoptions.ModeRBAC) {
		mustGetRESTOptions := func(resource string) generic.RESTOptions {
			...
			// F:\go-source\kubernetes-1.4.6\pkg\registry\generic\options.go
		...
		// role.NewRegistry == F:\go-source\kubernetes-1.4.6\pkg\registry\role\registry.go  应用层接口
		//  NewREST 中通过工厂方法创建真正的etcd的存储 F:\go-source\kubernetes-1.4.6\pkg\registry\role\etcd\etcd.go
	...
	// F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\options\server_run_options.go
...
//见到master了  F:\go-source\kubernetes-1.4.6\pkg\master\master.go
// new 后init，装饰器模式组装  认证 ，https  等handler ，，，installGroups暴露服务
// 初始化etcd
...
//  F:\go-source\kubernetes-1.4.6\pkg\controller\informers\factory.go 
	...
	// master是pkg\genericapiserver\genericapiserver.go的 子类
	//F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\genericapiserver.go
