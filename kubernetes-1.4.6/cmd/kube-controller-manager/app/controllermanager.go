...
func Run(s *options.CMServer) error {
	...
	//连接 apiServer F:\go-source\kubernetes-1.4.6\pkg\client\unversioned\helper.go
	...
	//事件广播  F:\go-source\kubernetes-1.4.6\pkg\client\record\event.go
	...
	// 选举  F:\go-source\kubernetes-1.4.6\pkg\client\leaderelection\leaderelection.go
...
func StartControllers(s *options.CMServer, kubeClient *client.Client, kubeconfig *restclient.Config, stop <-chan struct{}, recorder record.EventRecorder) error {
	//F:\go-source\kubernetes-1.4.6\pkg\controller\informers\factory.go
	//F:\go-source\kubernetes-1.4.6\pkg\client\cache\controller.go  只知道是通知器,存储，控制器
	// 从etcd 中同步数据  客户端集
	...
	//endpoint是什么概念  后端服务， 里边有serviceController
	//F:\go-source\kubernetes-1.4.6\pkg\controller\endpoint\endpoints_controller.go
	//  所有的client 都连接api-server
...
//云服务提供商  这个仔细看看  F:\go-source\kubernetes-1.4.6\pkg\cloudprovider\plugins.go    
...
// 集群地址    CIDR无类别域间路由
	...
	//节点管理  从注册中心同步节点信息 F:\go-source\kubernetes-1.4.6\pkg\controller\node\nodecontroller.go
	// 只看到 分配了些路由，加入队列
	...
	//节点控制器   处理节点 增删改查， crash
	nodeController.Run()
	time.Sleep(wait.Jitter(s.ControllerStartInterval.Duration, ControllerStartJitter))
	//服务控制器   从注册中心同步service queue   F:\go-source\kubernetes-1.4.6\pkg\controller\service\servicecontroller.go
	...
	if s.AllocateNodeCIDRs && s.ConfigureCloudRoutes {
		if cloud == nil {
			...
			//路由控制器   从注册中心同步路由规则，创建路由器
			//F:\go-source\kubernetes-1.4.6\pkg\controller\route\routecontroller.go
...
//资源配额 api-server的client
	...
	//启动配额      pkg\controller\resourcequota\resource_quota_controller.go
	...
	//服务发现    F:\go-source\kubernetes-1.4.6\pkg\client\typed\discovery\discovery_client.go
	// 这个服务发现应该是   注册中心了
	...
	//命名空间管理 F:\go-source\kubernetes-1.4.6\pkg\controller\namespace\namespace_controller.go
	...
	if containsVersion(versions, groupVersion) && found {
		...
		if containsResource(resources, "horizontalpodautoscalers") {
			...
			//水平扩容  pkg\client\clientset_generated\internalclientset\clientset.go
			hpaClient := clientset.NewForConfigOrDie(restclient.AddUserAgent(kubeconfig, "horizontal-pod-autoscaler"))
			//监控相关了  F:\go-source\kubernetes-1.4.6\pkg\controller\podautoscaler\metrics\metrics_client.go
			...
			//自动伸缩   pkg\controller\podautoscaler\horizontal.go  水平扩容了  参数 hpaClient 连接的配置中心
			//猜想  发命令给api-server -> 调度 -> service,node，proxy 执行
		...
		if containsResource(resources, "daemonsets") {
			...
			//  后台程序  F:\go-source\kubernetes-1.4.6\pkg\controller\daemon\daemoncontroller.go  创建deamon pod
		...
		if containsResource(resources, "jobs") {
			...
			//F:\go-source\kubernetes-1.4.6\pkg\controller\job\jobcontroller.go  也是创建pod
...
//容量管理  用插件的形式 连接云存储
	...
	//持久化管理  F:\go-source\kubernetes-1.4.6\pkg\controller\volume\persistentvolume\controller_base.go
	volumeController := persistentvolumecontroller.NewPersistentVolumeController(
		...
		alphaProvisioner, // 就是 云存储

		// 探测 插件    挂载点，回收器  
		ProbeControllerVolumePlugins(cloud, s.VolumeConfiguration),//  cloud  F:\go-source\kubernetes-1.4.6\pkg\cloudprovider\plugins.go
		cloud,
		s.ClusterName,
		nil, // volumeSource
		nil, // claimSource
...
//F:\go-source\kubernetes-1.4.6\pkg\controller\volume\attachdetach\attach_detach_controller.go
	...
	if containsVersion(versions, groupVersion) && found {
		...
		if containsResource(resources, "certificatesigningrequests") {
			...
			resyncPeriod := ResyncPeriod(s)()
			// 证书，权限控制  先不看了   F:\go-source\kubernetes-1.4.6\pkg\controller\certificates\controller.go
			//  从api-server 同步证书过来
	...
	if len(s.ServiceAccountKeyFile) > 0 {
		...
		if err != nil {
			...
			// 同步token   serviceAccount  F:\go-source\kubernetes-1.4.6\pkg\controller\serviceaccount\tokens_controller.go
...
//服务账号??? F:\go-source\kubernetes-1.4.6\pkg\controller\serviceaccount\serviceaccounts_controller.go
...
//垃圾回收
	if s.EnableGarbageCollector {
		...
		//  垃圾回收  F:\go-source\kubernetes-1.4.6\pkg\controller\garbagecollector\garbagecollector.go
