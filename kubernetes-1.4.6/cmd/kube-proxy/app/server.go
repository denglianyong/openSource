...
func NewProxyServerDefault(config *options.ProxyServerConfig) (*ProxyServer, error) {
	...
	// Create a iptables utils. 防火墙相关
	//F:\go-source\kubernetes-1.4.6\pkg\util\exec\exec.go  封装操作系统命令接口
	execer := exec.New()
	// F:\go-source\kubernetes-1.4.6\pkg\util\dbus\dbus.go
	//  os 消息总线  D-bus
	dbus := utildbus.New()
	//F:\go-source\kubernetes-1.4.6\pkg\util\iptables\iptables.go
	// 真正干活的，执行防火墙命令
	...
	if config.OOMScoreAdj != nil {
		//F:\go-source\kubernetes-1.4.6\pkg\util\oom\oom_linux.go
	...
	if config.ResourceContainer != "" {
		...
		// F:\go-source\kubernetes-1.4.6\pkg\util\resourcecontainer\resource_container_linux.go
		//    runc 容器 docker 
	...
	// api server的 client   获取本节点的代理模式配置
	...
	if proxyMode == proxyModeIPTables {
	    ...
	    //F:\go-source\kubernetes-1.4.6\pkg\proxy\userspace\proxier.go
	...
	// are registered yet.
	//服务的配置  ----- 
//F:\go-source\kubernetes-1.4.6\pkg\proxy\config\config.go
	serviceConfig := proxyconfig.NewServiceConfig()
	//proxier，endpointsHandler  ==在F:\go-source\kubernetes-1.4.6\pkg\proxy\iptables\proxier.go
...
//  通过api-server监听etcd，同步事件到本地store，广播，交给代理的2个handler处理  ;; 重要的交通枢纽
//  list watch机制new中已经run
//F:\go-source\kubernetes-1.4.6\pkg\proxy\config\config.go
	proxyconfig.NewSourceAPI(
		client,
		config.ConfigSyncPeriod,
		//创建2个管道，来接收配置变更; 这个模块数据的源头
...
func (s *ProxyServer) Run() error {
	...
	// Birth Cry after the birth is successful  发布启动事件 
	s.birthCry()

	// Just loop forever for now...  F:\go-source\kubernetes-1.4.6\pkg\proxy\userspace\proxier.go
	//iptable  命令  放开防火墙管制
	s.Proxier.SyncLoop()  //
	return nil
}
