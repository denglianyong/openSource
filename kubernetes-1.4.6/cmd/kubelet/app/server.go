...
//文件系统挂载   F:\go-source\kubernetes-1.4.6\pkg\util\mount\mount.go
	mounter := mount.New()
	var writer kubeio.Writer = &kubeio.StdWriter{}
	if s.Containerized {
		glog.V(2).Info("Running kubelet in containerized mode (experimental)")
		// F:\go-source\kubernetes-1.4.6\pkg\util\mount\nsenter_mount.go
	...
	if s.ContainerRuntime == "docker" {
		// docker的client 层层传递下去最终到  kubernetes-1.4.6\pkg\kubelet\dockertools\docker_manager.go
		//处理pod  通过docker client 回调  
		// F:\go-source\kubernetes-1.4.6\pkg\kubelet\dockertools\docker.go 实现
...
func run(s *options.KubeletServer, kubeDeps *kubelet.KubeletDeps) (err error) {
	...
	if kubeDeps == nil {
		...
		if s.CloudProvider != v1alpha1.AutoDetectCloudProvider {
			// F:\go-source\kubernetes-1.4.6\pkg\cloudprovider\plugins.go
	...
	// 连接api server
		...
		if err == nil {
			...
			// F:\go-source\kubernetes-1.4.6\pkg\client\clientset_generated\internalclientset\typed\core\unversioned\core_client.go
			// 事件客户端
       ...
       //连接docker   重点关注 新建docker client回调函数
		...
		kubeDeps.KubeClient = kubeClient //连接api server
		kubeDeps.EventClient = eventClient
	}

	if kubeDeps.CAdvisorInterface == nil {
		// 监控系统  F:\go-source\kubernetes-1.4.6\pkg\kubelet\cadvisor\cadvisor_linux.go
	...
	if kubeDeps.ContainerManager == nil {
		...
		//cgroup subsystem  相关  F:\go-source\kubernetes-1.4.6\pkg\kubelet\cm\container_manager_linux.go
...
func kubeconfigClientConfig(s *options.KubeletServer) (*restclient.Config, error) {
	if s.RequireKubeConfig {
		...
		// F:\go-source\kubernetes-1.4.6\pkg\client\unversioned\clientcmd\merged_client_builder.go
...
// 将命令行参数转化成配置，其中包含 api-server 列表
...
func RunKubelet(kubeCfg *componentconfig.KubeletConfiguration, kubeDeps *kubelet.KubeletDeps, runOnce bool, standaloneMode bool) error {
	...
	//F:\go-source\kubernetes-1.4.6\pkg\client\record\event.go
	eventBroadcaster := record.NewBroadcaster()
	// 发布事件 并广播  //F:\go-source\kubernetes-1.4.6\pkg\client\record\event.go
	...
	if kubeDeps.EventClient != nil {
		...
		//接受事件   加些过滤器  
		//参数中传入 sink  EventSinkImpl = \pkg\client\clientset_generated\internalclientset\typed\core\unversioned\event_expansion.go
		// EventClient  ===  	\pkg\client\clientset_generated\internalclientset\typed\core\unversioned\core_client.go
		//kubeDeps.EventClient.Events("")   在上一行的core_client.go  62行实现  
	...
	//CreateAndInitKubelet=builder  F:\go-source\kubernetes-1.4.6\pkg\kubelet\kubelet.go  创建真正的kuberlet
	...
	//F:\go-source\kubernetes-1.4.6\pkg\kubelet\kubelet.go  321行，从api-server中获取pod
	...
	if runOnce {
		//F:\go-source\kubernetes-1.4.6\pkg\kubelet\runonce.go
...
func startKubelet(k kubelet.KubeletBootstrap, podCfg *config.PodConfig, kubeCfg *componentconfig.KubeletConfiguration, kubeDeps *kubelet.KubeletDeps) error {
	// start the kubelet 实现在？？  F:\go-source\kubernetes-1.4.6\pkg\kubelet\kubelet.go 1173行
	//初始化模块，然后处理chan来的命令   podCfg里的chan 有api-server来的消息  syncLoop 进入循环 处理pod事件
...
//F:\go-source\kubernetes-1.4.6\pkg\kubelet\kubelet.go  同步pod ;;  NewMainKubelet 生成一堆服务
