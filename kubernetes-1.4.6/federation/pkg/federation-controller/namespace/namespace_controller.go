...
func NewNamespaceController(client federation_release_1_4.Interface) *NamespaceController {
	//事件发布器  F:\go-source\kubernetes-1.4.6\pkg\client\record\event.go
	...
	nc := &NamespaceController{
		...
		// 流控 指数退避 F:\go-source\kubernetes-1.4.6\pkg\util\flowcontrol\backoff.go
		namespaceBackoff:      flowcontrol.NewBackOff(5*time.Second, time.Minute),
		eventRecorder:         recorder,
	}

	// Build delivereres for triggering reconcilations.
	//F:\go-source\kubernetes-1.4.6\federation\pkg\federation-controller\util\delaying_deliverer.go 
	...
	nc.namespaceInformerStore, nc.namespaceInformerController = cache.NewInformer(
		...
		// 事件回调函数 F:\go-source\kubernetes-1.4.6\federation\pkg\federation-controller\util\handlers.go
...
func (nc *NamespaceController) Run(stopChan <-chan struct{}) {
	// namespaceInformerController 是个informer，list watch 事件驱动框架
...
func (nc *NamespaceController) deliverNamespace(namespace string, delay time.Duration, failed bool) {
	if failed {
		// 	namespaceBackoff= 流控 指数退避 F:\go-source\kubernetes-1.4.6\pkg\util\flowcontrol\backoff.go
