...
func Run(s *options.ServerRunOptions) error {
	...
	//F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\default_storage_factory_builder.go
	// 返回  F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\storage_factory.go
	...
	if modeEnabled(genericoptions.ModeRBAC) {
		mustGetRESTOptions := func(resource string) generic.RESTOptions {
			...
			//  Decorator: generic.UndecoratedStorage 定义在 F:\go-source\kubernetes-1.4.6\pkg\registry\generic\storage_decorator.go 
			// F:\go-source\kubernetes-1.4.6\federation\registry\cluster\etcd\etcd.go 回调 Decorator创建etcd3
		...
		// F:\go-source\kubernetes-1.4.6\pkg\registry\role\registry.go   crud 流程分析见 单机版
	...
	// F:\go-source\kubernetes-1.4.6\pkg\genericapiserver\options\server_run_options.go   api-server的本机地址
	client, err := s.NewSelfClient()
	if err != nil {
		glog.Errorf("Failed to create clientset: %v", err)
	}
	//  F:\go-source\kubernetes-1.4.6\pkg\controller\informers\factory.go 实现对api-server的list watch
	sharedInformers := informers.NewSharedInformerFactory(client, 10*time.Minute)
	//F:\go-source\kubernetes-1.4.6\pkg\admission\init.go
	pluginInitializer := admission.NewPluginInitializer(sharedInformers)

//权限插件  F:\go-source\kubernetes-1.4.6\pkg\admission\chain.go
